...
const Sidebar = () => {
    ...
    const { sidebarOpen, darkMode, toggleTheme } = useUI();
    ...
    return (
        <aside
            className={`w-64 bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800 h-screen fixed left-0 top-0 flex flex-col z-20 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
        >
            ...
            <nav className="flex-1 p-4 space-y-1">
                {navItems.map((item) => (
                    <NavLink
                        key={item.path}
                        to={item.path}
                        end={item.path === '/dashboard'} // strictly match dashboard root
                        className={({ isActive }) =>
